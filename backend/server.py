...
def read_events():
    ...
    # testing some events here 
    id = '8tyd3jwkvrean3e7ki0jep3ye'
    print("printing calender fetched by id")
...
'''
1. Combine both nylas_mail and nylas_events and create a single backend.
2. Combine both frontends and have 2 buttons to toggle between emails and events.
3. Route to Update the existing event
4. Route to fetch the threads / messages when an email is selected.
5. Send email using a lebel.
'''